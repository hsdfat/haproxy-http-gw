#!/bin/bash
# Dynamic Backend Registration/Deregistration Test Script
...
GATEWAY_CONTAINER="test-gateway-1"
...
if ! curl -sf "$GATEWAY_API/health" > /dev/null 2>&1; then
    ...
    echo "Make sure the gateway is running: $COMPOSE_CMD up -d"
    exit 1
fi
...
register_backend "test-backend-1" "srv1" "backend-server-1" "9000"
sleep 3
...
register_backend_multi_server "test-backend-2" \
    "srv1:backend-server-1:9000" \
    "srv2:backend-server-2:9000" \
    "srv3:backend-server-3:9000"
sleep 3
...
for i in {1..5}; do
    ...
    register_backend "test-backend-rapid-$i" "srv-$i" "backend-server-1" "9000"
    sleep 2
...
register_backend "concurrent-backend-1" "srv1" "backend-server-1" "9000" &
PID1=$!
register_backend "concurrent-backend-2" "srv2" "backend-server-2" "9000" &
PID2=$!
register_backend "concurrent-backend-3" "srv3" "backend-server-3" "9000" &
PID3=$!
...
register_backend "reconfig-backend" "srv1" "backend-server-1" "9000"
sleep 3
